...
export function gregorianToBengali(year, month, day, monthStarts = null) {
  ...
  if (monthStarts && typeof monthStarts === 'object') {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      if (i < 11) {
        ...
        if (yearData[nextMonthKey]) {
          ...
          // If so, check if it's Day 1 of next month
          if (normalizedGregorian.getTime() === normalizedNextStart.getTime()) {
            ...
            // Otherwise, it belongs to this month - fall through
          }
    ...
    // Use the normalizeDate function and normalizedGregorian already defined above
    ...
    // According to bengalicalendar.com, if Sankranti occurs early in the day (before midnight IST),
    // the Sankranti date itself is Day 1 of the Bengali month.
    // If Sankranti occurs later in the day (afternoon/evening IST), that Gregorian date
    // is still part of the previous Bengali month, and the next day is Day 1.
    // Since we can't determine exact Sankranti timing from dates alone, we follow bengalicalendar.com's
    // convention: the Sankranti date is Day 1 if it's included in the month (using >=).
    // So if JSON says "2026-01-15":
    // - Jan 15 = Sankranti day = Day 1 (0 days since Sankranti + 1 = Day 1)
    // - Jan 16 = Day 2 (1 day since Sankranti + 1 = Day 2)
    // - Jan 22 = Day 8 (7 days since Sankranti + 1 = Day 8)
    // Therefore: bengaliDay = daysSinceMonthStart + 1
    ...
    let maxDay;
    if (nextMonthStart) {
      // Calculate month length using Bisuddha Siddhanta Panjika algorithm
      const normalizedNextStart = normalizeDate(nextMonthStart);
      const normalizedMonthStartNorm = normalizeDate(monthStart);
      const { monthLength } = calculateMonthLengthBisuddhaSiddhanta(
...
export function getFestivals(bengaliMonth, bengaliDay, bengaliYear = null, festivalsData = null) {
  ...
  if (bengaliYear && festivalsData && typeof festivalsData === 'object') {
    ...
    if (yearData && typeof yearData === 'object') {
      ...
      if (yearData[monthKey] && typeof yearData[monthKey] === 'object' && yearData[monthKey][dayKey]) {
