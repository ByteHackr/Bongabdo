...
import { calculateMonthLengthBisuddhaSiddhanta } from './bisuddhaSiddhanta.js';
...
export function computeWestBengalMonthView({ monthStarts, month, anchorDate }) {
    ...
    // Determine Bengali year from the mapping year
    // Bengali year = Gregorian year - 593 (approximately)
    const bengaliYear = Number(yearKeyUsed) - 593;

    // Calculate month length using Bisuddha Siddhanta Panjika algorithm
    const { firstDay, lastDay, monthLength } = calculateMonthLengthBisuddhaSiddhanta(
      monthStart,
      nextMonthStart,
      month,
      bengaliYear
    );
    const daysInMonth = Math.max(1, Math.min(monthLength || 30, 32));
    ...
    if (prevMonthStartStr) {
        ...
        // For previous month, use same Bengali year (or year - 1 if month is Boishakh)
        const prevBengaliYear = month === 0 ? bengaliYear - 1 : bengaliYear;
        const { monthLength: prevDaysRaw } = calculateMonthLengthBisuddhaSiddhanta(
          prevMonthStart,
          monthStart,
          prevMonthIndex,
          prevBengaliYear
    ...
    return { daysInMonth, firstDayOfWeek, prevMonthDays, yearKeyUsed, firstDay };
}
