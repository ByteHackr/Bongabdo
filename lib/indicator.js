        ...
        for (let r = 0; r < 6; r++) {
            ...
            for (let c = 0; c < 7; c++) {
                ...
                const festivals = (showFestivals && inMonth && dayNum && this._festivalsData)
                    ...
                    : [];
                const festivalText = festivals.length > 0 ? festivals[0] : '';

                if (festivalText) {
                    const cellBox = new St.BoxLayout({
                        vertical: true,
                        style_class: 'bongabdo-calendar-day-cell',
                    });
                    const dayLabel = new St.Label({ text: dayText, style_class: style });
                    const festivalLabel = new St.Label({
                        text: festivalText,
                        style_class: 'bongabdo-calendar-day-festival',
                    });
                    festivalLabel.clutter_text.ellipsize = 2; // Pango.EllipsizeMode.END
                    festivalLabel.clutter_text.line_wrap = true;
                    festivalLabel.clutter_text.line_wrap_mode = 2; // Pango.WrapMode.WORD_CHAR
                    cellBox.add_child(dayLabel);
                    cellBox.add_child(festivalLabel);
                    row.add_child(cellBox);
                } else {
                    row.add_child(new St.Label({ text: dayText, style_class: style }));
                }
            }
